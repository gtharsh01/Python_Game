...
ver=int(input("enter the vertices"))
from_node=0
to_node=0
root = tk.Tk()
...
def floyd_warsh(adj,f,t):
    ...
    for i in range(0,ver):
        if(adj[i][k]<0):
            messagebox.showinfo("OUTPUT","consists of negative cycle")
            flag=0
    if(flag==1):
        if(adj[f-1][t-1]==999999999):
            messagebox.showinfo("OUTPUT","no connection")
        else:
            short=adj[f-1][t-1]
            messagebox.showinfo("OUTPUT","shortest distance %d" %short)
def create_adj(new):
    adj=np.ones((ver,ver))
